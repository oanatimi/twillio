...
class ConversationMemory {
  ...
  const timeExpressions = {
    ...
    '13:00': 'ora treisprezece',
    '14:00': 'ora paisprezece',
    '15:00': 'ora cincisprezece',
    '16:00': 'ora șaisprezece'
  };
  ...
  // Service detection - MORE EXPLICIT
  let detectedService = null;
  
  // ITP
  if (lowerText.includes('itp') || lowerText.includes('inspectie tehnica')) {
    detectedService = 'ITP';
  } 
  // Vulcanizare
  else if (lowerText.includes('vulcanizare') || lowerText.includes('anvelope') || 
           lowerText.includes('roți') || lowerText.includes('roti') || 
           lowerText.includes('cauciuc') || lowerText.includes('pneuri')) {
    detectedService = 'VULCANIZARE';
  } 
  // Climatizare
  else if (lowerText.includes('clima') || lowerText.includes('climatizare') || 
           lowerText.includes('aer conditionat') || lowerText.includes('ac ')) {
    detectedService = 'CLIMA';
  } 
  // Mecanica
  else if (lowerText.includes('mecanică') || lowerText.includes('mecanica') || 
           lowerText.includes('motor') || lowerText.includes('frâne') || 
           lowerText.includes('frane') || lowerText.includes('direcție') || 
           lowerText.includes('directie') || lowerText.includes('diagnoza') ||
           lowerText.includes('reparatie') || lowerText.includes('reparație')) {
  ...
  // Check if this is a question (don't auto-save)
  const isQuestion = lowerText.includes('ce oră') || 
                     lowerText.includes('la ce oră') || 
                     lowerText.includes('aveți disponibilitate') ||
                     lowerText.includes('aveti disponibilitate') ||
                     lowerText.includes('când pot') ||
                     lowerText.includes('cand pot');
  
  if (isQuestion) {
    console.log(`⚠️ Este întrebare - nu salvăm automat`);
    // Salvează doar serviciul dacă e detectat
    if (detectedService && !this.hasAppointment(streamSid)) {
      this.setPartialBooking(streamSid, { service: detectedService });
      console.log(`📝 Saved service from question: ${detectedService}`);
    }
    return false;
  }
  
  // Save partial booking
  if ((detectedDay || detectedTime || detectedService) && !this.hasAppointment(streamSid)) {
    const partialInfo = {};
    if (detectedDay) partialInfo.day = detectedDay;
    if (detectedTime) partialInfo.time = detectedTime;
    if (detectedService) partialInfo.service = detectedService;
    
    this.setPartialBooking(streamSid, partialInfo);
    console.log(`🤖 AUTO-PARTIAL: ${JSON.stringify(partialInfo)}`);
    return true;
  }
  ...
  // DOAR de la user, NICIODATĂ de la assistant
  if (role === 'user') {
  ...
  this.appointments.set(streamSid, {
    ...
    time: time,
    service: service,
    confirmed: true, // NEW FLAG
    savedToBackend: false, // NEW FLAG
    timestamp: Date.now()
  });
